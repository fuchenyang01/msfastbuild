	...
	public class msfastbuild
	{
		...
		public class ObjectListNode
		{
            ...
            public void ToStringSplitPart(ref int ActionNumber, string BaseDir, List<string> InputFiles, StringBuilder RetStr)
            {
                ...
                if (CustomBuildIndex > -1)
                {
                    RetStr.AppendFormat("\t.PreBuildDependencies  = 'custombuild_{0}'\n", CustomBuildIndex);
                }
                else if (!string.IsNullOrEmpty(PreBuildBatchFile))
                {
		...
		static private void GenerateBffFromVcxproj(string Config, string Platform)
		{
            ...
            foreach (var Item in CustomBuilds)
            {
                ...
                var CustomBuildBatchFile = Path.Combine(ActiveProject.DirectoryPath, Path.GetFileNameWithoutExtension(ActiveProject.FullPath) + $"_custom_{CustomBuildIndex + 1}.bat");
